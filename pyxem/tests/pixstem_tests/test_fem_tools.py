...
class TestFemCalc:
    ...
    def test_result_shapes(self):
        s = dd.get_generic_fem_signal(probe_x=5, probe_y=2, image_x=49, image_y=50)
    ...
    def test_result_values(self):
        ...
        assert femresult["RadialInt"].data.sum() > femresult["RadialAvg"].data.sum()
...
class TestFemPlot:
    def test_plot_square_fem(self):
        s = dd.get_simple_fem_signal()
        femresult = femt.fem_calc(s, centre_x=25, centre_y=25, show_progressbar=False)
        fig = femt.plot_fem(s, femresult, lowcutoff=10, highcutoff=120, k_cal=None)
    ...
    def test_plot_parameters_kcal_none(self):
        ...
        fig = femt.plot_fem(
            s, femresult, lowcutoff=lowcutoff, highcutoff=highcutoff, k_cal=k_cal
        )

        assert fig.axes[1].xaxis.properties()["data_interval"][0] == xaxis[lowcutoff]
        assert (
            fig.axes[1].xaxis.properties()["data_interval"][1] == xaxis[highcutoff - 1]
        )
    ...
    def test_plot_parameters_kcal_finite(self):
        ...
        xaxis = 2 * np.pi * k_cal * np.arange(0, len(femresult["RadialAvg"].data))
