...
class PolarDiffraction2D(Signal2D):
    ...
    def get_radial_correlation(self, mask=None, normalize=True,
                               radial_bin=1, angular_bin=1,
        ...
        radial_bin: int
            The bin factor in the radial direction. Improves signal but loses radial resolution
        angular_bin: int
            The bin factor in the angular direction. Improves signal but loses angular resolution
        kwargs: dict
        ...
        if self.axes_manager.signal_shape() == np.shape(mask): # for a static mask
            correlation = self.map(correlation, mask=mask,
                                      radial_bin=radial_bin, angular_bin=angular_bin,
                                      normalize=normalize,inplace=inplace, **kwargs)
            ...
            correlation = self._map_iterate(correlation,iterating_kwargs=(('mask', mask),),
                                            normalize=True,radial_bin=1, angular_bin=1,
                                            inplace=inplace, **kwargs)
        if inplace:
            self.set_signal_type("Signal2D")  # It should already be a Signal 2D object...
            self.axes_manager.signal_axes[0].scale = self.axes_manager.signal_axes[0].scale * radial_bin
            self.axes_manager.signal_axes[1].scale = self.axes_manager.signal_axes[1].scale * angular_bin
            self.axes_manager.signal_axes[1].name = "Radial Correlation, $/phi$"

        else:
            correlation.axes_manager.signal_axes[0].scale = self.axes_manager.signal_axes[0].scale * radial_bin
            correlation.axes_manager.signal_axes[1].scale = self.axes_manager.signal_axes[1].scale * angular_bin
            correlation.axes_manager.signal_axes[1].name = "Radial Correlation, $/phi$"
        return correlation

    def get_radial_power(self, mask=None, normalize=True,
                         radial_bin=1, angular_bin=1,
                         inplace=False,
                         ** kwargs):
        """ Returns the power spectrum of the angular auto-correlation function
         ...
         Parameters
         ---------------
         signal_mask: Numpy array or Signal2D
        ...
        if self.axes_manager.signal_shape() == np.shape(mask): # for a static mask
            pow = self.map(power, mask=mask,
                                      radial_bin=radial_bin, angular_bin=angular_bin,
                                      normalize=normalize,inplace=inplace)
        elif self.axes_manager.shape() == np.shape(mask):  # for a changing mask
            mask = np.reshape(mask, newshape=(-1, *reversed(self.axes_manager.signal_shape)))
            pow = self._map_iterate(power,iterating_kwargs=(('mask', mask),),
                                            normalize=True,radial_bin=1, angular_bin=1,
                                            inplace=inplace)
        if inplace:
            self.set_signal_type("Signal2D")  # It should already be a Signal 2D object...
            ...
            pow.axes_manager.signal_axes[0].scale = self.axes_manager.signal_axes[0].scale * radial_bin
