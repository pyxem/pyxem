...
def difference_of_gaussians(
    image, sigma1, sigma2, order=0, mode="reflect", cval=0.0, truncate=4.0
):
    filtered1 = gaussian_filter(
        image, sigma=sigma1, order=order, mode=mode, cval=cval, truncate=truncate
    )
    filtered2 = gaussian_filter(
        image, sigma=sigma2, order=order, mode=mode, cval=cval, truncate=truncate
    )

    return filtered1 - filtered2


def difference_of_gaussians_lazy(
    image,
    sigma1,
    sigma2,
    order=0,
