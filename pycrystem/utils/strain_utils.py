...
def construct_displacement_gradient(dp, ref):
    ...
    """
    D = BaseSignal(np.ones((dp.axes_manager.navigation_shape[1],
...
def _transform_basis2D(D, angle):
    ...
    angle : float
        The anti-clockwise rotation angle between the diffraction x/y axes and the
        x/y axes in which the tensor is to be specified.
...
def polar_decomposition(dp, D, side='right'):
    ...
    R = BaseSignal(np.ones((dp.axes_manager.navigation_shape[1],
                            dp.axes_manager.navigation_shape[0],
                            3,3)))
    R.axes_manager.set_signal_dimension(2)
    U = BaseSignal(np.ones((dp.axes_manager.navigation_shape[1],
                            dp.axes_manager.navigation_shape[0],
                            3,3)))
