...
class TestFitEllipse:
    ...
    def test_find_parameters(self):
        ...
        xC, yC, semi_len0, semi_len1, rot, eccen = ra._get_ellipse_parameters(
                ellipse_parameters)
        assert approx(xC, abs=0.1) == 0.
        assert approx(yC, abs=0.1) == 0.
        assert approx(semi_len0, abs=1.) == axis2
        assert approx(semi_len1, abs=1.) == axis1
    ...
    def test_fit_single_ellipse_to_signal(self):
        ...
        output[0].plot()
        assert approx(output[1], abs=0.01) == 0.
        assert approx(output[2], abs=0.01) == 0.
        assert approx(output[3], abs=0.6) == 60
        assert approx(output[4], abs=0.6) == 60
        assert approx(output[6], abs=0.000001) == 1.
